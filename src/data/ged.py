...
class GEDDataset(InMemoryDataset):
    ...
    datasets = {
        ...
        'PTC': {
            ...
            'extract': extract_zip,
            'pickle': '1nNKFSztVb2hXrsvUi20DH85JIbB7IXx3',
        },
    }
    ...
    def process(self):
        ...
        for r_path, p_path in zip(self.raw_paths, self.processed_paths):
            ...
            for i, idx in enumerate(ids[-1]):
                ...
                # elif self.name == 'PTC':
                #     self.types = [str(i) for i in range(23)] # I assume there are 23 types in PTC?
                #     # Not sure using it as a string is correct
                #     x = torch.zeros(data.num_nodes, dtype=torch.long)
                #     for node, info in gr.nodes(data=True):
                #         x[int(node)] = self.types.index(info['type'])
                #     data.x = fn.one_hot(
...
if __name__ == "__main__":
    dataset_name = 'LINUX'
    dl = GEDDataset(root='./data/{}'.format(dataset_name), name=dataset_name)